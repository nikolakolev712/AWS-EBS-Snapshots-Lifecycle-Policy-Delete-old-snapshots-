...
def filter_active_inactive(owner_id, snapshots_csv, data_d=get_volumes(json_file_)):
    ...
    data_active = [i for i in data_v_1 if i["Snapshots/OwnerId"] == owner_id and i["Snapshots/VolumeId"] in data_d]
    data_inactive = [i for i in data_v_1 if i["Snapshots/OwnerId"] == owner_id and i["Snapshots/VolumeId"] not in data_d]

    data_active_snapshots = pd.DataFrame(data_active)
    data_inactive_snapshots = pd.DataFrame(data_inactive)

    data_active_snapshots.to_csv("active_volumes_test.csv", index=False)
    data_inactive_snapshots.to_csv("inactive_volumes_test.csv", index=False)
...
def filter_date(date_cvs, start_date_, owner_id):
    ...
    data_snapshots.to_csv("date_formated_test.csv", index=False)
